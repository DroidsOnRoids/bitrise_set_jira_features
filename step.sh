...
tasks=""
tasks_list=($(git log --pretty=format:"%s" | grep "$project_prefix[0-9]{1,5}" -o -E | sort -u -r --version-sort | sed -e 's/^/'${ESCAPED_URL}'/'))

for (( i=0 ; i<${#tasks_list[*]} ; ++i ))
do
	tasks+="${tasks_list[$i]}"
...
envman add --key FEATURES_FROM_COMMITS --value "$tasks"
